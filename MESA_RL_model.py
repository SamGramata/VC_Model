...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 11 07:24:05 2022
...
from ddpg_10 import Agent, ReplayBuffer, OUActionNoise, CriticNetwork, ActorNetwork
...
class VC(Agent):
    ...
    #Calculates expected variance for the whole portfolio 
    ...
    #Gets expected return on a Portfolio, using projection on how will the EPI evolve
    ...
    #Calculates expected return on a Portfolio, without using EPI projection
    ...
    def get_reward(self, action, startup):
        ...
        if self.Fund_life_stage <= (Fund_maturity - Early_startup_exit):   
            #We want to avoid negative actions
            if action < 0:
                return torch.tensor([-100*(-action[0])])
            #Allocating less than 0.005 of endowement implies no invesment and hence 0 reward
            if 0<action <0.005:
                return torch.tensor([0])
        #If action in this range then VC invests in startup
            if 0.01<=action<=1 and action <= self.Endowement:
        ...
        #If action in this range then VC invests in startup
                ...
                if action>1:
                    return torch.tensor([-100*action[0]])
        #VC gets punished for investing too late
            ...
            for i in self.Screening_prospects:
                if i[1] == "Early" and self.Fund_life_stage >= (Fund_maturity - Early_startup_exit):
                    ...
                    if 0.005<=act<=1 and float(act) <= self.Endowement:
                        ...
                        self.Portfolio.append(i+[float(act)])
                    ...
                    print(act)
                    #print(act_1)
                    print("This is endowmwnet left")
                    print(self.Endowement)
                    print(reward)
                    print(obs)
                    print(new_state)
                    print("This is Fund life stage")
                    print(self.Fund_life_stage)
                    print("Time till end of stage E")
                    print(self.Time_till_end_of_investment_stage_E)
                    print("Time till end of stage L")
                    print(self.Time_till_end_of_investment_stage_L)
                    print("This is size")
                    print(self.Portfolio_size)
                    print("This is effort left for scr")
